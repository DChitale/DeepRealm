...
const STACK_CONFIG = [
  ...
  { rotate: -2, x: 15, y: 5,  scale: 0.96, opacity: 0.6  },
  { rotate:  2, x: 30, y: 10, scale: 0.92, opacity: 0.35 },
  { rotate:  6, x: 45, y: 15, scale: 0.88, opacity: 0.15 },
];
...
const TwilightZone = () => {
  ...
  const applyStack = useCallback((topIndex, animate = true) => {
    cardsRef.current.forEach((card, i) => {
      ...
      if (animate) {
        gsap.to(card, { x: cfg.x, y: cfg.y, scale: cfg.scale, rotate: cfg.rotate, opacity: cfg.opacity, duration: 0.65, ease: 'expo.out' });
      } else {
        gsap.set(card, { x: cfg.x, y: cfg.y, scale: cfg.scale, rotate: cfg.rotate, opacity: cfg.opacity });
      }
    });
  }, [cards.length]);
  ...
  useLayoutEffect(() => { applyStack(0, false); }, [applyStack]);
  ...
  useLayoutEffect(() => {
    ...
    const d = Draggable.create(topCardEl, {
      type: 'x,y', edgeResistance: 0.75,
      onDragStart() { if (isAnimating.current) return; gsap.to(topCardEl, { scale: STACK_CONFIG[0].scale * 1.04, duration: 0.15 }); },
      onDrag() { gsap.set(topCardEl, { rotate: this.x * 0.06 }); },
      onDragEnd() {
        if (isAnimating.current) return;
        if (Math.abs(this.x) > 110 || Math.abs(this.y) > 110) {
          isAnimating.current = true;
          const vx = this.x > 0 ? 1 : -1, vy = this.y > 0 ? 1 : -1;
          gsap.to(topCardEl, {
            x: this.x + vx * 500, y: this.y + vy * 300, opacity: 0, rotate: this.x * 0.15,
            duration: 0.45, ease: 'power2.in',
            onComplete: () => {
              const next = (currentIndex + 1) % cards.length;
              gsap.set(topCardEl, { x: STACK_CONFIG[cards.length-1].x, y: STACK_CONFIG[cards.length-1].y + 20, scale: STACK_CONFIG[cards.length-1].scale, rotate: STACK_CONFIG[cards.length-1].rotate, opacity: 0, zIndex: 0 });
          ...
          gsap.to(topCardEl, { x: 0, y: 0, scale: STACK_CONFIG[0].scale, rotate: STACK_CONFIG[0].rotate, duration: 0.7, ease: 'elastic.out(1, 0.7)' });
        }
      },
    })[0];
    ...
    cardsRef.current.forEach((card, i) => {
      if (!card) return;
      gsap.set(card, { pointerEvents: i !== currentIndex ? 'none' : 'auto' });
    });
    return () => { d?.kill(); };
  ...
  return (
    <section
      id="twilight-zone"
      ref={containerRef}
      className="min-h-[160vh] w-full relative flex items-center py-32 overflow-hidden bg-[#0e1317] select-none -mt-px"
    >
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12 w-full z-20 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        <div ref={contentRef} className="max-w-xl text-left">
          <span className="text-xs font-bold tracking-[0.5em] uppercase text-blue-400 mb-4 block font-mono">{zoneData.depth}</span>
          ...
          <p className="text-xl text-white/70 leading-relaxed mb-6 font-sans font-light">{zoneData.description}</p>
          <div className="flex items-center gap-4 text-blue-400/60 font-mono text-[10px] tracking-widest uppercase mb-12">
            <div className="w-12 h-px bg-blue-400/20" /><span>SWIPE DATA PACKETS</span><div className="w-12 h-px bg-blue-400/20" />
          </div>
        </div>
          ...
          {cards.map((fact, index) => (
            <div key={index} ref={el => { cardsRef.current[index] = el; }}
              ...
              <div className="w-full aspect-square mb-6 relative pointer-events-none flex items-center justify-center bg-black/20 rounded-[2rem] overflow-hidden">
                <img src={fact.image} alt={fact.title} className="w-full h-full object-contain drop-shadow-[0_20px_40px_rgba(30,58,138,0.5)]" draggable={false} />
              </div>
              <div className="flex flex-col gap-3 pointer-events-none">
                <h3 className="text-2xl md:text-3xl font-instrument font-black text-white uppercase leading-tight tracking-tight">{fact.title}</h3>
                <p className="text-sm text-white/40 leading-relaxed font-sans font-light">{fact.content}</p>
              </div>
              <div className="mt-auto pt-6 border-t border-white/5 flex justify-between items-center opacity-40 pointer-events-none">
                <span className="text-[9px] font-mono text-blue-400 tracking-widest uppercase">TWILIGHT.DATA_{index + 1}</span>
